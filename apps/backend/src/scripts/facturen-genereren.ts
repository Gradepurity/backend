...
export default async function facturenGenereren({
  ...
  for (const o of factuurbaar) {
    ...
    fs.writeFileSync(htmlPad, factuurHtml(o, entry, totaal), "utf8");
    if (edge) {
      try {
        execFileSync(edge, [
          "--headless",
...
function factuurHtml(o: any, entry: RegisterEntry, orderTotaal: number): string {
  ...
  for (const it of o.items ?? []) {
    ...
    regels.push(rij(it.product_title ?? it.title, aantal, excl / aantal, excl, btw));
  }
  ...
  if (verzendIncl > 0) {
    ...
    regels.push(rij("Verzendkosten", 1, excl, excl, btw));
  ...
  if (Math.abs(rest) >= 0.01) {
    ...
    regels.push(rij(rest < 0 ? "Korting" : "Toeslag", 1, excl, excl, btw));
  }
...
<title>Factuur ${entry.nummer}</title>
<style>
...
<body><div class="vel">
  <div class="kop"><div class="merk">${esc(BEDRIJF.naam)}</div><h1>Factuur</h1></div>
  <div class="blokken">
    <div><div class="label">Factuur aan</div>${klantRegels.join("<br>")}<br>${esc(o.email ?? "")}</div>
    <div><div class="label">Gegevens</div>
      Factuurnummer: <b>${entry.nummer}</b><br>
      Factuurdatum: ${entry.datum}<br>
      Ordernummer: #${o.display_id}
    </div>
  </div>
  <table>
    <tr><th>Omschrijving</th><th class="r">Aantal</th><th class="r">Stukprijs excl.</th><th class="r">Excl. btw</th><th class="r">Btw 21%</th></tr>
    ${regels.join("\n    ")}
  </table>
  <div class="totalen">
    <div><span>Totaal excl. btw</span><span>${eur(totaal.excl)}</span></div>
    <div><span>Btw 21%</span><span>${eur(totaal.btw)}</span></div>
    <div class="eind"><span>Totaal</span><span>${eur(orderTotaal)}</span></div>
  </div>
  <div class="voet">
    ${esc(BEDRIJF.naam)} · ${esc(BEDRIJF.adres)}<br>
