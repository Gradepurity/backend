...
/**
 * Surgical add of a single GradePurity product (bpc-157-tb-500 — de voorgemengde
 * "Wolverine"-blend) to the live catalog WITHOUT the destructive delete/rebuild
 * the full seed does. Idempotent: skips if the handle already exists.
 */
const TARGET_SLUG = "bpc-157-tb-500";
...
export default async function addBpcTb({
  container,
}: {
  ...
  logger.info("BPC-157 + TB-500 toegevoegd aan live catalogus. ✅");
}
